...
from src.app.domain.ports.use_cases.tutoring_session import (
    TutoringSessionServiceInterface,
)
from src.app.domain.ports.units_of_work.tutoring_session import (
    TutoringSessionUnitOfWorkInterface,
)
...
def _handle_response_failure(
    ...
    if teacher_code and student_code_:
        return ResponseFailure(
            ...
            message={
                "detail": f"Student with code code {student_code_} and Teacher with code {teacher_code} not found"
            },
        )
    if student_code_:
        return ResponseFailure(
    ...
    return ResponseFailure(ResponseTypes.RESOURCE_ERROR, message={"detail": f"ERROR"})
...
class TutoringSessionService(TutoringSessionServiceInterface):
    ...
    async def _create(
        self, tutoring_session: tutoring_session_schema.TutoringSessionCreate
    ) -> Union[ResponseSuccess, ResponseFailure]:
        try:
            with self.uow:
                student_exist = utils.student_exists(
                    student_code=tutoring_session.student_code
                )
                teacher_exist = utils.teacher_exists(
                    teacher_code=tutoring_session.teacher_code
                )
                if not student_exist and not teacher_exist:
                    return _handle_response_failure(
                        student_code_=tutoring_session.student_code,
                        teacher_code=tutoring_session.teacher_code,
                    )
    ...
    def _get_tutoring_sessions_for_student(
        self, student_code: str
    ) -> Union[ResponseSuccess, ResponseFailure]:
        with self.uow:
            student_exist = utils.student_exists(student_code=student_code)
            if not student_exist:
                return _handle_response_failure(student_code_=student_code)
            tutoring_sessions = (
                self.uow.tutoring_sessions.get_tutoring_sessions_for_student(
                    student_code
                )
            )
            if not tutoring_sessions:
                return _handle_response_failure(
                    message={
                        "detail": f"No tutoring sessions for student {student_code}"
                    }
                )
            if tutoring_sessions:
                return ResponseSuccess(tutoring_sessions)
            return _handle_response_failure(
                message={
                    "detail": f"Error getting tutoring sessions for student {student_code}"
                }
            )

    def _get_tutoring_sessions_for_teacher(
        ...
        with self.uow:
            ...
            if not tutoring_sessions:
                return _handle_response_failure(
                    message={
                        "detail": f"No tutoring sessions for teacher {teacher_code}"
                    }
                )
            if tutoring_sessions:
                return ResponseSuccess(tutoring_sessions)
            return _handle_response_failure(
                message={
                    "detail": f"Error getting tutoring sessions for teacher {teacher_code}"
                }
            )
